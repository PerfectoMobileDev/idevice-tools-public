...
plist_t plist_new_date(int32_t sec, int32_t usec)
{
	...
	data->_timeval.tv_sec = sec;
	data->_timeval.tv_usec = usec;
...
static void plist_get_type_and_value(plist_t node, plist_type * type, void *value, uint64_t * length)
{
    ...
    *length = data->length;
    ...
    case PLIST_DATE:
		...
		((struct timeval*)value)->tv_sec = data->_timeval.tv_sec;
		((struct timeval*)value)->tv_usec = data->_timeval.tv_usec;
        break;
    case PLIST_ARRAY:
    case PLIST_DICT:
    default:
...
int plist_data_compare(const void *a, const void *b)
{
    ...
    case PLIST_DICT:
        ...
        break;
    case PLIST_DATE:
		if (!memcmp(&(val_a->_timeval), &(val_b->_timeval), sizeof(struct timeval)))
            return TRUE;
        else
            return FALSE;
    default:
...
static void plist_set_element_val(plist_t node, plist_type type, const void *value, uint64_t length)
{
    ...
    case PLIST_DATA:
        ...
        break;
    case PLIST_DATE:
		data->_timeval.tv_sec = ((struct timeval*)value)->tv_sec;
		data->_timeval.tv_usec = ((struct timeval*)value)->tv_usec;
        break;
    case PLIST_ARRAY:
    case PLIST_DICT:
    default:

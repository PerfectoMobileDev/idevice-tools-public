...
static plist_t parse_date_node(char *bnode, uint8_t size)
{
	...
	data->_timeval.tv_sec = (long)time_real;
	data->_timeval.tv_usec = (time_real - (long)time_real) * 1000000;
...
static plist_t parse_data_node(char *bnode, uint64_t size)
{
	...
	//printf("DBG: parse_data_node - %d, size - %llu \n",data->buff,size);
...
static void* copy_plist_data(const void* src)
{
    ...
    case PLIST_UINT:
        ...
        break;
    case PLIST_DATE:
		dstdata->_timeval.tv_sec = srcdata->_timeval.tv_sec;
		dstdata->_timeval.tv_usec = srcdata->_timeval.tv_usec;
        break;
    case PLIST_REAL:
...
void plist_to_bin(plist_t plist, char **plist_bin, uint32_t * length)
{
    ...
    for (i = 0; i < num_objects; i++)
    {
        ...
        case PLIST_DICT:
            ...
            break;
        case PLIST_DATE:
			write_date(bplist_buff, data->_timeval.tv_sec + (double)data->_timeval.tv_usec / 1000000);
            break;
        case PLIST_UID:

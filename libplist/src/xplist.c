 ...
 */
static char *format_string(const char *buf, size_t len, int signedCols, int depth)
{
    if (!buf || !(len > 0)) return NULL;
    int colw = depth + signedCols + 1;
    int nlines = len / signedCols + 1;
    char *new_buf = NULL;
    int i = 0;
    int j = 0;
	...
	unsigned cols = (unsigned) signedCols;
...
static void node_to_xml(node_t* node, void *xml_struct,int fShowAddresses)
{
    ...
    case PLIST_DATE:
        tag = XPLIST_DATE;
        {
			time_t time = (time_t)node_data->_timeval.tv_sec;
	...
	if(fShowAddresses) {
		char addr[13];
		sprintf(addr,"[0x%llu]",(uint64_t)node_data);
		xmlNodeAddContent(xstruct->xml, BAD_CAST(addr));
	}
    xmlNodeAddContent(xstruct->xml, BAD_CAST("\n"));
    if (val) {
        free(val);
    ...
    if (isStruct)
    {
        ...
        while ((ch = node_iterator_next(ni))) {
            node_to_xml(ch, &child,fShowAddresses);
        }
...
static void xml_to_node(xmlNodePtr xml_node, plist_t * plist_node)
{
    ...
    for (node = xml_node->children; node; node = node->next)
    {
        ...
        if (!xmlStrcmp(node->name, XPLIST_DATE))
        {
			...
			data->_timeval.tv_sec = (long)time;
			data->_timeval.tv_usec = 0;
...
void plist_to_xml(plist_t plist, char **plist_xml, uint32_t * length)
{
    ...
    node_to_xml((node_t *)plist, &root,0);
...
void plist_to_xml_with_addresses(plist_t plist, char **plist_xml, uint32_t * length)
{
    xmlDocPtr plist_doc = NULL;
    xmlNodePtr root_node = NULL;
    struct xml_node root = { NULL, 0 };
    ...
    node_to_xml((node_t *)plist, &root,1);

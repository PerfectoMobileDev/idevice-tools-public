...
static bool str_is_udid(const char* str)
{
	...
	/* verify length */
	if (/*length != 40*/false) return false;
...
static void parse_opts(int argc, char **argv)
{
        ...
        while (1) {
                c = getopt_long(argc, argv, "hU:li:u:g:La:r:R:o:d:I:P:f", longopts, (int *) 0);
                if (c == -1) {
                        break;
                }
                ...
                case 'l':
                        cmd = CMD_LIST_APPS;
						list_apps_mode = 1;
						//log("ideviceinstaller: list_apps_mode\n");
                        break;
                case 'i':				
						cmd = CMD_INSTALL;
                        install_mode = 1;
                        appid = strdup(optarg);
						//log("ideviceinstaller: install_mode\n");
                        break;
                case 'u':
                        uninstall_mode = 1;
						cmd = CMD_UNINSTALL;
                        appid = strdup(optarg);
						//log("ideviceinstaller: uninstall_mode\n");
                        break;
				case 'I': 
						usbmuxd_ipv4(optarg);
					break;
                ...
                case 'L':
						...
						//log("ideviceinstaller: list_archives_mode\n");
                        break;
                case 'a':
						cmd = CMD_ARCHIVE;
                        archive_mode = 1;
...
{
	#ifdef WIN32
	SetDumpPrefix(L"IdeviceInstaller");
	CRASH_GUARD_BLOCK_START
#endif
		//init_log_file(stderr);
		//log("ideviceinstaller: start!");
		...
		//log("ideviceinstaller: client initialized");
		...
		//log("ideviceinstaller: service initialized");
	nperr = np_client_new(phone, service, &np);
#ifdef HAVE_LIBIMOBILEDEVICE_1_1_5
	if (service) {
...
run_again:
		...
		//log("ideviceinstaller: after starting installation_proxy");
	...
	//log("ideviceinstaller: after connect to installation_proxy");
        setbuf(stdout, NULL);
        ...
        if (list_apps_mode) {
			//log("ideviceinstaller: list mode");
                int xml_mode = 0;
				...
				//log("ideviceinstaller: got apps from device");
                instproxy_client_options_free(client_opts);
                if (err != INSTPROXY_E_SUCCESS) {
                ...
                if (!apps || (plist_get_node_type(apps) != PLIST_ARRAY)) {
                        fprintf(stderr,
                                        "ERROR: instproxy_browse returned an invalid plist!\n");
						res = -1;
                        goto leave_cleanup;
                }
                if (xml_mode) {
                        char *xml = NULL;
                        uint32_t len = 0;
						//log("ideviceinstaller: building xml and output it \n");
...
#ifdef HAVE_LIBIMOBILEDEVICE_1_1
					instproxy_error_t res = instproxy_install(ipc, pkgname, client_opts, NULL, NULL);
#else
						...
						* Some applications do not display "Install - Complete" although they install (NP-13071)
						*/
						if (res != INSTPROXY_E_SUCCESS)
						{
							if (bundleidentifier){
								fprintf(stderr, "bundleidentifier %s\r", bundleidentifier);
							}
							if (ipc) {
							...
							err = instproxy_browse(ipc, client_opts, &apps);
							if (err != INSTPROXY_E_SUCCESS) {
								//log("ERROR: instproxy_browse returned %d\n", err);
								res = -1;
								goto leave_cleanup;
							}

							int apps_number_new = plist_array_get_size(apps);
							fprintf(stderr, "apps_number_new %d\n", apps_number_new);
							...
							for (int i = 0; i < apps_number_new; i++) {
								...
								if (!s_appid) {
									...
									fprintf(stderr, "s_appid %s\n", s_appid);
									if (bundleidentifier){
										if (strcmp(s_appid, bundleidentifier) == 0) {
							...
							plist_free(apps);
							//log("stop log\n");
							if (successfully_installed == false) {
...
leave_cleanup:
		...
		fflush(NULL);
		// PATCH - sleep for a 1 sec to let IosDeviceBridge to read from stdout
		Sleep(1000);
        return res;
#ifdef WIN32

	...
	if ((ret == LOCKDOWN_E_SUCCESS) && service && (service->port > 0)) {
		if (diagnostics_relay_client_new(device, service, &diagnostics_client) != DIAGNOSTICS_RELAY_E_SUCCESS) {
			...
			switch (cmd) {
				...
				break;
				case CMD_RESTART: // DIAGNOSTICS_RELAY_ACTION_FLAG_WAIT_FOR_DISCONNECT set instead 0 on 21/12/2014 - check for hanging
				...
				break;
				case CMD_SHUTDOWN: // DIAGNOSTICS_RELAY_ACTION_FLAG_WAIT_FOR_DISCONNECT set instead 0 on 21/12/2014 - check for hanging

...
{
	char *cdir = NULL;
	size_t *cdirBufSize = NULL;
	_dupenv_s(&cdir, cdirBufSize, varname);
	return cdir;
}
...
#endif
	strcat_s(__config_dir, DIR_SEP_S);
	strcat_s(__config_dir, LIBIMOBILEDEVICE_CONF_DIR);
...
#ifndef WIN32
#include <assert.h>
unsigned int fopen_s(FILE **f, const char *name, const char *mode) {
	unsigned int ret = 0;
	assert(f);
	*f = fopen(name, mode);
 ...
 *
 * @return USERPREF_E_SUCCESS on success and USERPREF_E_INVALID_CONF or USERPREF_E_UNKNOWN_ERROR if no device record is given or if it has already
 *         been saved previously.
 */
...
static userpref_error_t userpref_gen_keys_and_cert(void)
{
	...
	root_key_pem.data = (unsigned char *)gnutls_malloc(root_key_export_size);
	host_key_pem.data = (unsigned char *)gnutls_malloc(host_key_export_size);
	...
	root_cert_pem.data = (unsigned char *)gnutls_malloc(root_cert_export_size);
	host_cert_pem.data = (unsigned char *)gnutls_malloc(host_cert_export_size);
...
{
	plist_dict_insert_item(pair_record, USERPREF_HOST_ID_KEY, plist_new_string(host_id));
...
userpref_error_t pair_record_set_item_from_key_data(plist_t pair_record, const char* name, key_data_t *value)
{
	...
	/* set new item */
	plist_dict_insert_item(pair_record, name, plist_new_data((char*)value->data, value->size));

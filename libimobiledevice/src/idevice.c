...
idevice_error_t idevice_get_device_list(char ***devices, int *count)
{
	...
	*count = 0;
	...
	if (dev_list == NULL) // oopsss
	{
		debug_info("ERROR: Failed to get device list (null) \n", __func__);
		return IDEVICE_E_UNKNOWN_ERROR;
	}
...
idevice_error_t idevice_new(idevice_t * device, const char *udid)
{
	...
	if (res > 0) 
	{
		...
		memset(dev->vers, 0, sizeof(int)*2);
...
static idevice_error_t internal_connection_send(idevice_connection_t connection, const char *data, uint32_t len, uint32_t *sent_bytes)
{
	...
	if (connection->type == CONNECTION_USBMUXD)
	{
		...
		if (res < 0)
		{
			...
			printf("internal_connection_send: usbmuxd_send failed\n");
			return IDEVICE_E_UNKNOWN_ERROR;
		}
		return IDEVICE_E_SUCCESS;
	} else {
...
size_t internal_connection_ReceiveBytesSockTimeout(idevice_connection_t connection, char * buffer, size_t length, unsigned int timeout)
{
	uint32_t received;
...
#ifdef WIN32
		res = usbmuxd_recv_timeout((int)(long)connection->data,  &buffer[receivedTotal], length - receivedTotal, &received,timeout - (GetTickCount()-StartTime));
#else 
		res = usbmuxd_recv((int)(long)connection->data,  &buffer[receivedTotal], length - receivedTotal, &received);
#endif 
		if ((res == 0) && (received == 0)) {
			// EOF
			break;
			...
			debug_info("USBMux recv error %d\n",res);
			receivedTotal = -1; //NP-22866 : returning err to be handled in upper level(s)
			break;
		}
...
size_t internal_connection_ReceiveBytesSSL(idevice_connection_t connection, char * buffer, int length)
{
	int received, receivedTotal = 0;

	do {
		// Try to read up to length
		received = SSL_read(connection->ssl_data->session, &buffer[receivedTotal], length - receivedTotal);
		if (received <= 0) {
			// Read failed, check if theres an SSL error
			int ret = SSL_get_error(connection->ssl_data->session, received);
			debug_info("SSL_read error (%x)\n", ret);
			break;
		}
...
static idevice_error_t internal_connection_receive_timeout(idevice_connection_t connection, char *data, uint32_t len, uint32_t *recv_bytes, unsigned int timeout)
{
	...
	if (connection->type == CONNECTION_USBMUXD) 
	{
		*recv_bytes = internal_connection_ReceiveBytesSockTimeout(connection, data, len,timeout);
...
idevice_error_t idevice_connection_receive_timeout(idevice_connection_t connection, char *data, uint32_t len, uint32_t *recv_bytes, unsigned int timeout)
{
	...
	if (connection->ssl_data) {
#ifdef HAVE_OPENSSL
		int received = internal_connection_ReceiveBytesSSL(connection, data, len);
...
static idevice_error_t internal_connection_receive(idevice_connection_t connection, char *data, uint32_t len, uint32_t *recv_bytes)
{
	...
	return internal_connection_receive_timeout(connection,data,len,recv_bytes,5000);
}
...
int g_USE_SSL_FALLBACK_METHOD = 0;
/**
...
{
	if (!connection || connection->ssl_data) return IDEVICE_E_INVALID_ARG;
	...
	if (pair_record) plist_free(pair_record);
	...
	// MICKEL- ssl falback method (starting iOS 10)
	SSL_CTX *ssl_ctx = SSL_CTX_new(g_USE_SSL_FALLBACK_METHOD ? SSLv3_method() : SSLv23_method());
	if (ssl_ctx == NULL) 
	{
...
idevice_error_t idevice_connection_disable_ssl(idevice_connection_t connection)
{
	...
	if (!connection->ssl_data)  	/* ignore if ssl is not enabled */ 
		return IDEVICE_E_SUCCESS;

...
mobile_image_mounter_error_t mobile_image_mounter_new(idevice_t device, lockdownd_service_descriptor_t service, mobile_image_mounter_client_t *client)
{
	...
	if (property_list_service_client_new(device, service, &plistclient) != MOBILE_IMAGE_MOUNTER_E_SUCCESS) {
		return MOBILE_IMAGE_MOUNTER_E_CONN_FAILED;
	}
...
leave_unlock:
  ...
  */
 mobile_image_mounter_error_t mobile_image_mounter_upload_image(mobile_image_mounter_client_t client, const char *image_type, size_t image_size, const char *image_signature, uint16_t signature_length, mobile_image_mounter_upload_cb_t upload_cb, void* userdata)
 {
	...
	if (image_signature && signature_length != 0)

...
static property_list_service_error_t internal_plist_receive_timeout(property_list_service_client_t client, plist_t *plist, unsigned int timeout)
{
	...
	*plist = NULL;
	if ((service_receive_with_timeout(client->parent, (char*)&pktlen, sizeof(pktlen), &bytes, timeout) == SERVICE_E_SUCCESS) && (bytes == 0)) {
	...
	if (bytes < 4) {
		...
		if (pktlen < (1 << 24)) { /* prevent huge buffers */
			...
			// DEBUG
#ifdef DEBUG_MEMORY_CRASHES
			if(!content) {
				printf("Memory Allocation Failed: pktlen is %lu, content is %d\n",pktlen,content);
				for(;;)
					Sleep(100);
			}
#endif 

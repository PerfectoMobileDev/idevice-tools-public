...
diagnostics_relay_error_t diagnostics_relay_query_ioregistry_entry(diagnostics_relay_client_t client, const char* name, const char* diag_class, plist_t* result)
{
	if (!client || (name == NULL && diag_class == NULL) || result == NULL)
		return DIAGNOSTICS_RELAY_E_INVALID_ARG;
	...
	if (diag_class)
		plist_dict_insert_item(dict,"EntryClass", plist_new_string(diag_class));

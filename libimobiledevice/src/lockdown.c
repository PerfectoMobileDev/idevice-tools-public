...
lockdownd_error_t lockdownd_query_type(lockdownd_client_t client, char **type)
{
	...
	if (type_node && (plist_get_node_type(type_node) == PLIST_STRING)) 
	{
		...
		if (type != NULL) 
			*type = typestr; /* return the type if requested */
		 else 
			free(typestr);
...
int lockdownd_client_get_os_version(idevice_t device)
{
	// TODO: init if 0!
	return device->vers[0];
}
/**
...
extern int g_USE_SSL_FALLBACK_METHOD;
...
lockdownd_error_t lockdownd_client_new_with_handshake(idevice_t device, lockdownd_client_t *client, const char *label)
{
	...
	if (pair_record)  pair_record_get_host_id(pair_record, &host_id);

	if (LOCKDOWN_E_SUCCESS == ret && !host_id) 
		ret = LOCKDOWN_E_INVALID_CONF;

	if (LOCKDOWN_E_SUCCESS == ret && !pair_record) 		/* attempt pairing */
	...
	if (device->vers[0] == 0)
	{
		...
		if (lockdownd_get_value(client_loc, NULL, "ProductVersion", &p_version) == LOCKDOWN_E_SUCCESS)
		{
			...
			if (s_version)
			{
				sscanf(s_version, "%d.%d", &device->vers[0], &device->vers[1]);
				free(s_version);
			}
		}
	}
	...
	int lockdown_SSL_session_failed = 0;
	if (LOCKDOWN_E_SUCCESS == ret) 
	{
		if (!host_id) 
		{
		...
		// MICKEL- check if lockdownd_start_session failed on SSL lockdown - try ssl falback method (starting iOS 10)
		int nSSLEnabled = 0;
		ret = lockdownd_start_session(client_loc, host_id, NULL, &nSSLEnabled);
		if (LOCKDOWN_E_SUCCESS != ret)
		{
			if (nSSLEnabled==1)
				lockdown_SSL_session_failed = 1;
	...
	if (lockdown_SSL_session_failed && !g_USE_SSL_FALLBACK_METHOD)
	{
		g_USE_SSL_FALLBACK_METHOD = 1;
		//fprintf(stderr, "g_USE_SSL_FALLBACK_METHOD.\n");
...
/**
resets global SSL mode flag
*/
void lockdown_reset_SSL_mode()
{
	g_USE_SSL_FALLBACK_METHOD = 0;
}
...
#ifndef __MICKEL_MERGE	
	if (!pair_record->host_id) userpref_get_host_id(&host_id_loc);
...
#ifndef __MICKEL_MERGE
	if (!pair_record->host_id) free(host_id_loc);
#endif // __MICKEL_MERGE
...
static lockdownd_error_t pair_record_generate(lockdownd_client_t client, plist_t *pair_record)
{
	...
	*pair_record = plist_new_dict();
	...
	if (system_buid) {
		plist_dict_insert_item(*pair_record, USERPREF_SYSTEM_BUID_KEY, plist_new_string(system_buid));
	}
...
static lockdownd_error_t lockdownd_do_pair(lockdownd_client_t client, lockdownd_pair_record_t pair_record, const char *verb)
{
	...
	} else 
	{
		...
		else 
		{
			...
			else return LOCKDOWN_E_INVALID_HOST_ID;
			*/

			// using Matthias patch preventing double Trust on Yosemite connections
			if (userpref_read_pair_record(client->udid, &pair_record_plist) != USERPREF_E_SUCCESS)
			{
				fprintf(stderr, "lockdownd: userpref_read_pair_record result not USERPREF_E_SUCCESS.\n");
				/*if (!pair_record_plist) 
					return LOCKDOWN_E_INVALID_CONF;*/
			}
		}
	...
	if (!pair_record_plist)
	{
		fprintf(stderr, "lockdownd: pair_record_plist == NULL. Exiting.\n");
		return LOCKDOWN_E_INVALID_CONF;
	}
	...
	dict = plist_new_dict();
	plist_dict_add_label(dict, client->label);
	plist_dict_insert_item(dict, "PairRecord", request_pair_record);
	plist_dict_insert_item(dict, "Request", plist_new_string(verb));
...
//	plist_dict_insert_item(dict, "PairingOptions", options);
	...
	if (options) plist_dict_insert_item(dict, "PairingOptions", plist_copy(options));
			...
			if (!strcmp("Unpair", verb)) {
				...
				if (!strcmp("Pair", verb)) {
					...
					if (extra_node && plist_get_node_type(extra_node) == PLIST_DATA) {
						...
						plist_dict_insert_item(pair_record_plist, USERPREF_ESCROW_BAG_KEY, plist_copy(extra_node));
					...
					if (wifi_node) {
						...
						plist_dict_insert_item(pair_record_plist, USERPREF_WIFI_MAC_ADDRESS_KEY, plist_copy(wifi_node));
						plist_free(wifi_node);
						wifi_node = NULL;
					}
									   ...
									   key_data_t * ohost_cert, key_data_t * oroot_cert)
{
	...
	if (GNUTLS_E_SUCCESS == gnutls_pem_base64_decode_alloc("RSA PUBLIC KEY", &public_key, &der_pub_key)) {
		...
		if (ASN1_SUCCESS == asn1_array2tree(pkcs1_asn1_tab, &pkcs1, NULL)) {
			...
			if (ASN1_SUCCESS == asn1_der_decoding(&asn1_pub_key, der_pub_key.data, der_pub_key.size, NULL)) {
				...
				modulus.data = (unsigned char *)gnutls_malloc(modulus.size);
				exponent.data = (unsigned char *)gnutls_malloc(exponent.size);
	...
	if (LOCKDOWN_E_SUCCESS == ret && 0 != modulus.size && 0 != exponent.size) {
		...
		gnutls_x509_privkey_t fake_privkey = NULL, root_privkey = NULL, host_privkey = NULL;
		...
		if (GNUTLS_E_SUCCESS ==
			...
			if (USERPREF_E_SUCCESS == uret) {
				...
				if (LOCKDOWN_E_SUCCESS == ret) {
					...
					dev_pem.data = (unsigned char *)gnutls_malloc(export_size);
					...
					if (USERPREF_E_SUCCESS == uret) {
						/* copy buffer for output */
						odevice_cert->data = (unsigned char *)malloc(dev_pem.size);
...
static lockdownd_error_t lockdownd_do_stop_service(lockdownd_client_t client, const char *identifier)
{
	if (!client || !identifier) return LOCKDOWN_E_INVALID_ARG;
	...
	uint16_t port_loc = 0;
	lockdownd_error_t ret = LOCKDOWN_E_UNKNOWN_ERROR;
	plist_t dict = dict = plist_new_dict();
	plist_dict_add_label(dict, client->label);
	plist_dict_insert_item(dict, "Request", plist_new_string("StopService"));
	...
	if (LOCKDOWN_E_SUCCESS != ret) return ret;
	
	ret = lockdownd_receive(client, &dict);

	if (LOCKDOWN_E_SUCCESS != ret) return ret;

	if (!dict) return LOCKDOWN_E_PLIST_ERROR;
...
static lockdownd_error_t lockdownd_build_start_service_request(lockdownd_client_t client, const char *identifier, int send_escrow_bag, plist_t *request)
{
	...
	if (send_escrow_bag) {
		...
		plist_dict_insert_item(dict, USERPREF_ESCROW_BAG_KEY, plist_copy(escrow_bag));
		plist_free(pair_record);
	}
...
lockdownd_error_t lockdownd_stop_service(lockdownd_client_t client, const char *identifier)
{
	return lockdownd_do_stop_service(client, identifier);
}

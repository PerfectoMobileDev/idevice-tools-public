...
#include <string>
#include <signal.h>
#include "usbmuxd.h" // usbmuxd public interface
#include "usbmuxd-proto.h" // usbmuxd protocol 
#include "sock_stuff.h" // socket utility functions
#include "utils.h" // misc utility functions
#include <thread>        
#include <mutex>   		 
...
#define NEW_UDID_PREFIX_LENGTH 8


static char 		__Remote_IP[20] 	= DEF_USBMUXD_IP;
static unsigned int __Remote_Port 		= DEF_USBMUXD_SOCKET_PORT;

//////////////////////////////////////////////////////////
//	validate_ipv4
//////////////////////////////////////////////////////////
bool validate_ipv4(const char* pbuff)
{
	unsigned char buff[20]="\0";
	unsigned char *a = &buff[0];
	int nCount = sscanf(pbuff, "%3hhd.%3hhd.%3hhd.%3hhd", a, a + 1, a + 2, a + 3);
	return  (nCount == 4);
}

void usbmuxd_ipv4(const char* pIP)
{
	if (validate_ipv4(pIP) && strcmp(__Remote_IP, pIP))
		strcpy(__Remote_IP, pIP);

}
void usbmuxd_port(unsigned int aPort)
{
	if (__Remote_Port != aPort)
		__Remote_Port = aPort;
}
...
//	usbmuxd_ipv4
//////////////////////////////////////////////////////////
const char* get_usbmuxd_ipv4(void)
{
	return ((const char*)__Remote_IP);
}
//////////////////////////////////////////////////////////
// usbmuxd_port
//	27015
//////////////////////////////////////////////////////////
unsigned int get_usbmuxd_port(void)
{
	return __Remote_Port;
}
//////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////
...
static usbmuxd_device_info_t *devices_find(int handle)
{
	...
	{
		if (dev && dev->handle == handle) return dev;
	} 
	ENDFOREACH
...
{
#ifdef __linux__
	// if run on linux localhost call "/var/run/usbmuxd" socket descriptor
	if(!strcmp(get_usbmuxd_ipv4(),DEF_USBMUXD_IP))
		return connect_unix_socket("/var/run/usbmuxd");
#endif
		return connect_socket(get_usbmuxd_ipv4(), get_usbmuxd_port());
}
...
static int receive_packet(int sfd, struct usbmuxd_header *header, void **payload, int timeout)
{
	...
	if (payload_size > 0) 
	{
		...
		do
		{
			int res = recv_buf_timeout(sfd, payload_loc + rsize, payload_size - rsize, 0, 5000);
			if (res < 0) break;
...
// ---------------------------------------------------------------------------------------------------
	if (payload && (payload_size > 0)) 
	{
		uint32_t ssize = 0;
		do {
			int res = send_buf(sfd, (char*)payload + ssize, payload_size - ssize);
			if (res < 0) break;
...
int get_next_event(int sfd, usbmuxd_event_cb_t callback, void *user_data)
{
	...
	if (hdr.message == MESSAGE_DEVICE_ADD) {
		...
		// MICKEL: patch iPhone Xs UDID  -----------------------------------------------------------------
		std::string strUDID(devinfo->udid);
		if (strUDID.length()>0 && strUDID.length() != 40)
		{
			if (strUDID.find("-") == std::string::npos) // trimmed UDID "0000802000196C2C3602002E"
			{
				strUDID.insert(NEW_UDID_PREFIX_LENGTH, "-"); // ->> 00008020-00196C2C3602002E
				strcpy(devinfo->udid, strUDID.c_str());
			}
		}
		// MICKEL: patch iPhone Xs UDID  -----------------------------------------------------------------
...
int usbmuxd_subscribe(usbmuxd_event_cb_t callback, void *user_data)
{
	...
	*/
	try {
		g_thread = std::thread(device_monitor, user_data);
	}
	catch (const std::exception &ex) {
		DEBUG(1, "Error in thread: %s\n", ex.what());
		return -1;
	}
...
retry:
	...
	if ((proto_version == 1) && (try_list_devices)) {
		if (send_list_devices_packet(sfd, use_tag) > 0) {
			plist_t list = NULL;
			if ((usbmuxd_get_result(sfd, use_tag, &res, &list) == 1) && (res == 0))
			{
				/*char* plistXML = NULL;
				uint32_t plistXMLSize = 0;
				plist_to_xml(list, &plistXML, &plistXMLSize);
				if (plistXMLSize>0) OutputDebugStringA(plistXML);*/
	...
	if (send_listen_packet(sfd, use_tag) > 0) {
		res = -1;
		// get response
		if ((usbmuxd_get_result(sfd, use_tag, &res, NULL)==1) && (res == 0)) {
			listen_success = 1;
		} else {
			g_mutex.unlock();  //UNLOCK;
			close_socket(sfd);
...
int usbmuxd_get_device_by_udid(const char *udid, usbmuxd_device_info_t *device)
{
	...
	for (i = 0; dev_list[i].handle > 0; i++) 
	{
		...
		if (/*!strcmp(udid, dev_list[i].udid)*/
			!strcmpnocase(udid, dev_list[i].udid))
		{
			device->handle = dev_list[i].handle;
			device->product_id = dev_list[i].product_id;
			strcpy(device->udid, dev_list[i].udid);
...
int usbmuxd_send(int sfd, const char *data, uint32_t len, uint32_t *sent_bytes)
{
	...
	if (len > 0) {
		char *tmp = NULL;
		int max_len = min(1000,len);
		if (DEBUG_DATA_HEX && *data != '<') {
			tmp = str2hexn(data, max_len);
		} else {
			// plist here, show as text
			tmp = (char *)malloc((max_len+1) * sizeof(char));
			strncpy(tmp, data, max_len * sizeof(char));
			tmp[max_len] = 0x00;
		}
